...
void ofApp::setup(){
    ...
    ofSeedRandom(23244); //23244 for three.
    ...
    ofSeedRandom(78); //78 for version three.
    ...
    otherPoints = evalCR(newControls, 1500);
    points = evalCR(controlPoints, 1500);
...
//        ofSetColor(ofColor::fromHsb(hue, saturation, brightness)); //Use for p&w
        ...
        auto l = (30  + (30 * sin(cos(ofDegToRad(i / 2.0 + ofGetFrameNum()))))) * scale;
        auto l2 = (45  + (45 * sin(cos(ofDegToRad(i / 2.0 + ofGetFrameNum()))))) * scale;
...
//        ofSetColor(ofColor::fromHsb(hue, saturation, brightness)); //Use for p&w
        ...
        ofDrawCircle(points[i].x, otherPoints[i].y, l2);
