...
void ofApp::draw(){
    ...
    for(int i = 0; i<points.size(); i++){
        ...
        float x = centre.x + (i/1.0) * cos(ofDegToRad(i)) * ofNoise((sFrame + ofGetFrameNum())/100.0) * scale;
        float y = centre.y + (i/1.0) * sin(ofDegToRad(i)) * ofNoise((sFrame + ofGetFrameNum())/1000.0) * scale;
        
        ofDrawCircle(points[i], 100 * scale + 50 * cos(ofDegToRad((i + sFrame + ofGetFrameNum())/1.0)) * scale); //50 * cos
